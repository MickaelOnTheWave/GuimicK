...
PACKAGE_VERSION=1
PACKAGE_NAME=guimick_$TASKMANAGER_VERSION-$PACKAGE_VERSION
...
buildSubProject()
{
	...
	cd $PROJECTSCRIPTS
	./MakeLinuxGccRelease.sh $version
	cd $CURRENTFOLDER
}
...
{
	cp $TASKTOOL_BIN_FOLDER/LinuxGccRelease/TaskTool $BIN_FOLDER/guimickTool
	cp $CONFTOOL_BIN_FOLDER/LinuxGccRelease/ConfigurationEditingTool $BIN_FOLDER/guimick
}
...
createDebianControlFile()
{
	...
	echo "Package: guimick" >> $CONTROL_FILE
	...
	echo "Description: GuimicK" >> $CONTROL_FILE
