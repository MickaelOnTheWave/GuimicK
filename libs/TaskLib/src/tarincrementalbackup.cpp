...
static const wstring defaultName = L"Tar Incremental Backup";
...
std::wstring TarIncrementalBackup::GetTypeName() const
{
   return defaultName;
}
      ...
      const int timeIndex)
{
   ...
   {
      const wstring backupSource = CreateBackupSourcePath(folderList[folderIndex].second);
      const int lastArchiveIndex = FindArchiveLastBackupIndex(backupSource);
      const wstring archiveName = CreateIncrementalArchiveName(backupSource, timeIndex, lastArchiveIndex);
      ...
      if (result)
         ...
         return new JobStatus(JobStatus::Error, L"Error while extracting");
   }
   else
...
void TarIncrementalBackup::RunRepositoryBackup(
      const wstring& source, const wstring& destination,
...
JobStatus* TarIncrementalBackup::RestoreBackupFromServer(const wstring& source,
                                                         const wstring& destination)
{
   TarTools tarTool(&target, debugManager);
...
void TarIncrementalBackup::CreateNewBackup(
      ...
      AbstractBackupJob::ResultCollection& results)
{
   const wstring finalDestination = repository + destination;
